---
layout: default
title: "Home"
---

<style>
  /* Reusable content blocks on the home page */
  .section-block {
    clear: both;
    margin-bottom: 2.2rem; /* Abstand zwischen Segmenten */
  }

  /* Consistent right-floated images inside blocks */
  .section-block img {
    float: right;
    margin: 0 0 0.4rem 1rem !important;
    border-radius: 12px;
    max-width: 100%;
    height: auto;
  }

  .section-block p:last-child {
    margin-bottom: 0;
  }

  h2 {
    margin-top: 0.2rem;
    margin-bottom: 0.7rem;
  }

/* Homepage hero sentence: centered, two lines, forest green */
.hero{
  text-align: center;
  font-size: clamp(1.15rem, 1.8vw, 1.55rem);
  line-height: 1.35;
  margin: 0.9rem auto 1.6rem;
  font-style: italic;
  font-weight: 500;
  max-width: 52rem;
  color: rgba(41, 86, 79, 1); /* from Inkscape: 29564fff */
}

.hero .sub{
  display: block;
  margin-top: 0.25rem;
}
</style>

<p class="hero">
  How people perceive and understand ecology across scales
  <span class="sub">— and how this relates to environmental beliefs</span>
</p>


# Welcome


<div class="section-block">
  <img
    src="/Lars.webp"
    alt="Lars Dietrich"
    width="200"
  >

  <p>
    I am a researcher working in the biology education group of Kerstin Kremer at Justus Liebig University Giessen, Germany. My work aims to strengthen ecology education by understanding how learners build scientific explanations and use them to make sense of the living world. I investigate ecological literacy and, as a particular focus, chemical mechanistic reasoning in biology — because chemical and biochemical mechanisms are foundational to modern life science and to ecological processes across scales. I collaborate on how learners work with authentic biodiversity datasets and how AI-supported learning environments can shape interpretation, judgement, and reasoning in ecological contexts. My background in plant ecology provides a scientific grounding for this work and helps me anchor learning research in authentic ecological phenomena. Alongside this, I remain active in plant ecology research on plant–environment interactions, especially the physiological effects of cold and drought.
  </p>
</div>

<div class="section-block">
  <h2>Research interests</h2>

<ul>
  <li>Ecological Literacy and Mechanistic Reasoning in Ecology</li>
  <li>AI in Teaching and Learning Biology/Ecology</li>
  <li>Data Literacy in Biodiversity Contexts</li>
  <li>Plant Awareness and Visual Perception</li>
  <li>Plant Physiological Ecology</li>
</ul>
</div>

<div class="section-block">
  <h2>Textbook</h2>

  <img
    src="/book.webp"
    alt="Chemie im Biologiestudium"
    width="200"
  >

  <p>
    Together with my colleague and friend Christian Schmidt, I co-authored a textbook Chemistry for Biologists, first published in November 2014 and now available in its second edition.
    The book introduces chemistry from the ground up, helping biology students in their early semesters build a solid and intuitive understanding of chemical principles and processes relevant to life sciences.
    Our goal was to create an accessible, step-by-step guide that makes chemistry clear and engaging for everyone entering the field of biology.<br><br>
    <strong>Schmidt C, Dietrich L (2022).</strong>
    <em>Chemie im Biologiestudium: Von Grund auf verständlich erklärt.</em>
    Second Edition. Springer Spektrum.
  </p>

  <p>
    Link:
    <a href="https://link.springer.com/book/10.1007/978-3-662-63416-5">
      https://link.springer.com/book/10.1007/978-3-662-63416-5
    </a>
  </p>
</div>

<div class="section-block" style="margin-bottom: 0;">
  <h2>Contact</h2>

  <p>
    <a href="mailto:&#108;&#97;&#114;&#115;&#46;&#100;&#105;&#101;&#116;&#114;&#105;&#99;&#104;&#64;&#117;&#110;&#105;&#45;&#103;&#105;&#101;&#115;&#115;&#101;&#110;&#46;&#100;&#101;">Email</a><br>
    Institution: Justus Liebig University Gießen (Germany)
  </p>
</div>
